...
def generate_report():
    ...
    try:
        ...
        # --- Format Output Message (Identical to previous version) ---
        output_parts = []
        ...
        for date_str in sorted(all_fetched_data["expo_weekend"].keys()):
            ...
            sorted_times_all = sorted(list(unique_times_all), key=lambda t: datetime.strptime(t, "%I:%M %p"))

            if unique_times_all:
                output_parts.append(f"✅ {formatted_date} : {' | '.join(sorted_times_all)}")
...
#         # Changed main header - removed bolding
#         output_parts.append("🏸 Badminton Court Availability (Next 7 Days) 🏸")
#         # output_parts.append(f"*(Data as of {datetime.now().strftime('%d %b %Y, %I:%M %p %Z')})*\n") # Keep commented out
        
#         # Weekday Report - removed bolding and added newline for spacing
#         output_parts.append("\n--- Weekdays (7 PM - 11 PM) ---")
        
#         # Expo Courts for Weekdays - added emoji, removed bolding
#         output_parts.append("\n🏟️ Expo Courts:")
...
#             if unique_times:
#                 # Changed separator and removed "Available timeslots:"
#                 output_parts.append(f"✅ {formatted_date} : {' | '.join(sorted_times)}")
#             else:
#                 # Removed bolding from date_str
#                 output_parts.append(f"❌ {formatted_date}: No timeslots found.")
        
#         # Sims Courts for Weekdays - added emoji, removed bolding
#         output_parts.append("\n🏟️ Sims Courts:")
...
#         # Weekend Report - removed bolding and added newline for spacing
#         output_parts.append("\n--- Weekends (11 AM - 10 PM) ---")

#         # Expo Courts for Weekends - added emoji, removed bolding
#         output_parts.append("\n🏟️ Expo Courts:")
...
#         # Sims Courts for Weekends - added emoji, removed bolding
#         output_parts.append("\n🏟️ Sims Courts:")
