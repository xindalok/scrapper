...
# This route handles requests to the /execute endpoint
@app.route('/execute', methods=['POST'])
def execute():
    # Print statement for debugging in Cloud Run logs
    print("DEBUG: /execute endpoint successfully hit in Flask app!") 
    
    # Define the simple JSON response
    response_data = {
        "message": "Hello from Cloud Run! Your Python script is responding!",
        "image": None # Keep this as None if no image is sent
    }
    
    # Print the data being returned (for Cloud Run logs)
    print(f"DEBUG: Returning JSON: {response_data}")
    
    # Return the JSON response with a 200 OK status
    return jsonify(response_data), 200

# This block ensures the Flask app runs when deployed on Cloud Run
